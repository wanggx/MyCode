...
class CollectCustomLayout: UICollectionViewLayout {
    ...
    override func collectionViewContentSize() -> CGSize {
        ...
        let sectionCount = self.collectView.numberOfSections()
        for i in 0..<sectionCount {
            let sectionHeight = self.getSectionTotalHeight(i)
    ...
    override func layoutAttributesForElementsInRect(rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        for count in 0..<sectionCount {
            let cellCount = self.collectView.numberOfItemsInSection(count)
            for i in 0..<cellCount {
                let indexPath =  NSIndexPath(forItem:i, inSection:count)
    ...
    override func layoutAttributesForItemAtIndexPath(indexPath: NSIndexPath) -> UICollectionViewLayoutAttributes! {
        ...
        let index:NSIndexPath = NSIndexPath(forItem: 0, inSection: 1)
        let cellWidth = CGFloat(Int((self.collectView.frame.width-self.collectView.spaceForCell(index))/2))
        let space = self.collectView.frame.width-cellWidth*2
        ...
        if indexPath.section == 0 {
            ...
            let firstHeight = self.getSectionTotalHeight(0)
            let startHeight = firstHeight + self.getSectionCellStartHeight(indexPath)
...
extension CollectCustomLayout {
    ...
    func getSectionTotalHeight(section:Int)->CGFloat {
        var totalHeight:CGFloat = 0
        if section == 0 {
            let cellCount = self.collectView.numberOfItemsInSection(section)
            ...
            let index:NSIndexPath = NSIndexPath(forItem: 0, inSection: 1)
            let otherIndex = NSIndexPath(forItem: 1, inSection: 1)
            
            let cellWidth = CGFloat(Int((self.collectView.frame.width-self.collectView.spaceForCell(index))/2))
            let space = self.collectView.frame.width-cellWidth*2
            
            let firstHeight:CGFloat = self.collectView.heightForCell(index) + self.collectView.spaceForCell(index)
    ...
    func getSectionCellStartHeight(indexPath:NSIndexPath)->CGFloat {
        ...
        if indexPath.section == 0 {
            ...
            let firstIndex:NSIndexPath = NSIndexPath(forItem: 0, inSection:1)
            let otherIndex:NSIndexPath = NSIndexPath(forItem: 1, inSection:1)
            
            let firstHeight:CGFloat = self.collectView.heightForCell(firstIndex) + self.collectView.spaceForCell(firstIndex)
